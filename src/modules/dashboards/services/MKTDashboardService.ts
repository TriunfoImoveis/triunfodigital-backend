  ...
  public async execute(): Promise<IRequestMKTDashboardDTO> {
    ...
    const mktSales = sales_sellers.map(sale => {
      ...
      return {
        ...
        'CONSTRUTORA': sale?.builder?.name || '',
        'CLIENTE VENDEDOR - NOME': sale?.client_seller?.name || '',
        'CLIENTE VENDEDOR - DATA DE NASCIMENTO': sale?.client_seller?.date_birth || '',
        'CLIENTE VENDEDOR - EMAIL': sale?.client_seller?.email || '',
        'CLIENTE VENDEDOR - TELEFONE': sale?.client_seller?.phone || '',
        'CLIENTE VENDEDOR - PROFISSAO': sale?.client_seller?.occupation || '',
        'CLIENTE VENDEDOR - ESTADO CIVIL': sale?.client_seller?.civil_status || '',
        ...
        'COOREDENADOR': sale?.user_coordinator?.name || '',
        'CAPTADORES': captivators,
        'VENDEDORES': selers,
        'SATUS': sale?.status,
        'TIPO PAGAMENTO': sale?.payment_type?.name,
        '1º PARCELA': sale?.installments[0]?.value || '',
        '1º PARCELA DT PAG': sale?.installments[0]?.pay_date || '',
        '2º PARCELA': sale?.installments[1]?.value || '',
        '2º PARCELA DT PAG': sale?.installments[1]?.pay_date || '',
        '3º PARCELA': sale?.installments[2]?.value || '',
        '3º PARCELA DT PAG': sale?.installments[2]?.pay_date || '',
        '4º PARCELA': sale?.installments[3]?.value || '',
        '4º PARCELA DT PAG': sale?.installments[3]?.pay_date || '',
        '5º PARCELA': sale?.installments[4]?.value || '',
        '5º PARCELA DT PAG': sale?.installments[4]?.pay_date || '',
      }
    })
