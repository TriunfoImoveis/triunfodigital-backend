...
class SellersDashboardService {
  ...
  public async execute(): Promise<IRequestMKTDashboardDTO> {
    ...
    const mktSales = sales_sellers.map(sale => {
      const directores = sale.users_directors
        ? sale.users_directors.map(director => director.name).join(',')
        : '';
      const captivators = sale.sale_has_captivators
        ? sale.sale_has_captivators.map(captivator => captivator.name).join(',')
        : '';
      const selers = sale.sale_has_sellers
        ? sale.sale_has_sellers.map(seller => seller.name).join(',')
        : '';
      return {
        ...
        'CLIENTE VENDEDOR - NOME': sale.client_seller
          ? sale.client_seller.name
          : '',
        'CLIENTE VENDEDOR - DATA DE NASCIMENTO': sale.client_seller
          ? sale.client_seller.date_birth
          : '',
        'CLIENTE VENDEDOR - EMAIL': sale.client_seller
          ? sale.client_seller.email
          : '',
        'CLIENTE VENDEDOR - TELEFONE': sale.client_seller
        ...
        'CLIENTE VENDEDOR - PROFISSAO': sale.client_seller
          ...
          : '',
        'CLIENTE VENDEDOR - ESTADO CIVIL': sale.client_seller
          ? sale.client_seller.civil_status
          : '',
        'CLIENTE COMPRADOR - NOME': sale.client_buyer.name,
        'CLIENTE COMPRADOR - ORIGEM':
          sale.client_buyer && sale.client_buyer.origin
            ? sale.client_buyer.origin.name
            : '',
        ...
        '1º PARCELA DT PAG': sale.installments[0]
          ? sale.installments[0].pay_date
          : '',
        '2º PARCELA': sale.installments[1] ? sale.installments[1].value : '',
        '2º PARCELA DT PAG': sale.installments[1]
          ? sale.installments[1].pay_date
          : '',
        '3º PARCELA': sale.installments[2] ? sale.installments[2].value : '',
        '3º PARCELA DT PAG': sale.installments[2]
          ? sale.installments[2].pay_date
          : '',
        '4º PARCELA': sale.installments[3] ? sale.installments[3].value : '',
        '4º PARCELA DT PAG': sale.installments[3]
          ? sale.installments[3].pay_date
          : '',
        '5º PARCELA': sale.installments[4] ? sale.installments[4].value : '',
        '5º PARCELA DT PAG': sale.installments[4]
          ? sale.installments[4].pay_date
          : '',
      };
    });
