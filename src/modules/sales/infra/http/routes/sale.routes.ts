...
saleRoutes.get(
  ...
  celebrate({
    [Segments.QUERY]: {
      ...
      status: Joi.string()
        .valid('NAO_VALIDADO', 'CAIU', 'PENDENTE', 'PAGO_TOTAL')
        .allow(''),
    },
  }),
...
saleRoutes.post(
  ...
  celebrate({
    [Segments.BODY]: {
      ...
      realty_ammount: Joi.number()
        .positive()
        .required()
        .messages(validatorFields({ name: "'valor da venda'" })),
      percentage_sale: Joi.number()
        .positive()
        .required()
      ...
      installments: Joi.array()
        ...
        .min(1)
        .required()
        .messages(validatorFields({ name: "'parcelas'", min: 1 })),
      subsidiary: Joi.string()
        .uuid()
        .required()
  ...
  celebrate({
    [Segments.BODY]: {
      ...
      client_buyer: Joi.string()
        ...
        .required()
        .messages(validatorFields({ name: "'cliente comprador'" })),
      client_seller: Joi.string()
        .uuid()
        .required()
        .messages(validatorFields({ name: "'cliente vendedor'" })),
      user_coordinator: Joi.string()
        .uuid()
        .messages(validatorFields({ name: "'coordenador'" }))
        .allow(null),
  ...
  celebrate({
    ...
    [Segments.BODY]: {
      ...
      builder: Joi.string()
        ...
        .messages(validatorFields({ name: "'construtora'" })),
      realty: Joi.object({
        enterprise: Joi.string().messages(
          validatorFields({ name: "'empreendimento'" }),
        ),
        unit: Joi.string().messages(validatorFields({ name: "'unidade'" })),
        state: Joi.string()
          .length(2)
          .uppercase()
          .messages(validatorFields({ name: "'estado'", max: 2 })),
        city: Joi.string().messages(validatorFields({ name: "'cidade'" })),
        neighborhood: Joi.string().messages(
          validatorFields({ name: "'bairro'" }),
        ),
        property: Joi.string()
          .uuid()
          .messages(validatorFields({ name: "'tipo de imóvel'" })),
      ...
      client_buyer: Joi.object({
        ...
        cnpj: Joi.when('client_type', {
          ...
          then: Joi.string()
            ...
            .required()
            .messages(
              validatorFields({ name: "'cnpj do comprador'", max: 14 }),
            ),
        }),
        email: Joi.string()
          .email()
        ...
        phone: Joi.string()
          .pattern(/^[0-9]{11,11}$/)
          .messages(
            validatorFields({ name: "'telefone do comprador'", max: 11 }),
          ),
        whatsapp: Joi.string().messages(
          validatorFields({ name: "'whatsapp do comprador'" }),
        ),
        date_birth: Joi.date()
          .iso()
          .messages(
        ...
        number_children: Joi.number()
          ...
          .min(0)
          .messages(
            validatorFields({ name: "'nº de filhos do comprador'", min: 0 }),
          ),
        gender: Joi.string()
      ...
      client_seller: Joi.object({
        name: Joi.string().messages(
          validatorFields({ name: "'nome do vendedor'" }),
        ),
        client_type: Joi.string()
          .valid('FISICA', 'JURIDICA')
          .messages(
        ...
        phone: Joi.string()
          .pattern(/^[0-9]{11,11}$/)
          .messages(
            validatorFields({ name: "'telefone do vendedor'", max: 11 }),
          ),
        whatsapp: Joi.string().messages(
          validatorFields({ name: "'whatsapp do vendedor'" }),
        ),
        date_birth: Joi.date()
          .iso()
          .messages(
            validatorFields({ name: "'data de nascimento do vendedor'" }),
          ),
        profession_id: Joi.string()
          .uuid()
        ...
        number_children: Joi.number()
          ...
          .min(0)
          .messages(
            validatorFields({ name: "'nº de filhos do vendedor'", min: 0 }),
          ),
        gender: Joi.string()
          .valid('MASCULINO', 'FEMININO', 'OUTRO')
          .messages(
...
saleRoutes.post(
  '/update-sale-subsidiary',
  celebrate({
    [Segments.BODY]: {
      subsidiaryId: Joi.string()
        .uuid()
        .required()
        .messages(validatorFields({ name: "'filial'" })),
    },
